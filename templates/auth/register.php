    ...
    if (!isset($_POST['csrf_token']) || !$auth->validateCsrfToken($_POST['csrf_token'])) {
        ...
        if (empty($username) || empty($email) || empty($password)) {
            ...
            if (!$passwordValidation['isValid']) {
                ...
                // Register user using Auth class
    ...
    <div class="w-full max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 p-6">
        ...
        <form method="POST" action="?action=register" class="space-y-6" aria-label="Registration form">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <button type="button" id="togglePassword" class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600">
                        <svg id="eyeIcon" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2シング458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                        </svg>
                    </button>
                </div>
                ...
                <!-- Password Errors -->
                <div id="passwordErrors" class="mt-2" aria-live="polite"></div>
            </div>
...
<script>
    ...
    const COMMON_PASSWORDS = new Set(<?php echo json_encode(array_slice(PasswordPolicy::$commonPasswords, 0, 20)); ?>);
    ...
    togglePasswordBtn.addEventListener('click', function() {
        ...
        if (type === 'text') {
            ...
            eyeIcon.innerHTML = `
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d Kri="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
            `;
        }
    });
